...
interface AgentComparisonAlignedProps {
  ...
  onSectionSelect: (sections: Record<string, { agentName: string; node: AtlasNode }>) => void;
  showBuilder: boolean;
...
const substituteVariables = (content: string, agentName: string, tokenSymbol: string): string => {
  ...
  const agentNames = ['Prysm', 'Spark', 'Grove', 'Keel', 'Launch Agent 3', 'Launch Agent 4'];
  const tokenSymbols = ['PRM', 'SPK', 'GROVE', 'KEEL', 'AGENT3', 'AGENT4'];
    ...
    <>
      ...
      <Box style={{
        ...
        {nodes.map((node, agentIndex) => {
          ...
          const isSelected = selectedSections[docNoSuffix]?.agentName === agentName;
          ...
          return (
            <Box key={agentIndex} style={{ overflow: 'hidden' }}>
              <Paper
                ...
                style={{
                  marginLeft: level * 12,
                  borderLeft: isSelected ? '3px solid var(--mantine-color-green-6)' : '3px solid var(--mantine-color-blue-6)',
                  overflow: 'hidden'
                }}
              >
        ...
        {showBuilder && (
          <Box style={{ overflow: 'hidden' }}>
            {selectedSections[docNoSuffix] ? (
              ...
              >
                {(() => {
                  ...
                  const substitutedContent = selectedNode.content
                    ? substituteVariables(selectedNode.content, builderAgentName, builderTokenSymbol)
                    : '';
              ...
              >
                <Text size="xs" c="dimmed" fs="italic">Select a section</Text>
              </Paper>
            )}
          </Box>
        )}
    ...
    <Box style={{ margin: 0, padding: 0 }}>
      ...
      {showBuilder && (
        <Box style={{
          ...
          {[...Array(1 + agents.length)].map((_, i) => <Box key={i} />)}
            ...
            <Paper p="sm" withBorder bg="dark.7">
              <Stack gap="xs">
                <TextInput
                  ...
                  onChange={(e) => onBuilderAgentNameChange(e.currentTarget.value)}
                  disabled={variablesCommitted}
                  size="xs"
                  styles={{
                    input: {
                      backgroundColor: variablesCommitted ? 'var(--mantine-color-dark-7)' : undefined
                    }
                  }}
                />
                <TextInput
                  ...
                  onChange={(e) => onBuilderTokenSymbolChange(e.currentTarget.value)}
                  disabled={variablesCommitted}
                  size="xs"
                  styles={{
                    input: {
                ...
                {!variablesCommitted ? (
                  <Button
                    ...
                    size="xs"
                    color="green"
                    disabled={!builderAgentName && !builderTokenSymbol}
                  >
                    Commit
                  </Button>
                ) : (
